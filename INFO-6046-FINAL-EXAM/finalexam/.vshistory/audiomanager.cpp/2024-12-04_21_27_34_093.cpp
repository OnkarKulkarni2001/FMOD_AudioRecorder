...
void AudioManager::GetRandomSound(FMOD::Sound** sound)
{
	...
	// Define a list of file paths for random sounds
	static const char* randomSoundPaths[] = {
		"sound1.wav",
		"sound2.wav",
		"sound3.wav",
		"sound4.wav"
	...
	int soundIndex = rand() % (sizeof(randomSoundPaths) / sizeof(randomSoundPaths[0]));

	// Load the sound
	FMOD_RESULT result = m_System->createSound(randomSoundPaths[soundIndex], FMOD_DEFAULT, nullptr, sound);
	FMODCheckError(result);
}