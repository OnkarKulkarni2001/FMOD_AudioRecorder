...
void AudioManager::EndRecording()
{
	...
	uint32_t sampleRate = 44100;  // Example: 44.1kHz
	uint16_t channels = 1;        // Mono
	uint16_t bitsPerSample = 16;  // 16-bit samples

	const size_t bufferSize = 1024;   // Buffer size (for simplicity)
	int16_t tempBuffer[bufferSize];   // Temporary buffer to hold audio data

	size_t totalDataSize = 0;

	// Read data from the RingBuffer and write it to the file
	while (true)  // Loop until all data is written
	{
		size_t availableData = m_RingBuffer.GetAvailableData();
		if (availableData == 0)
