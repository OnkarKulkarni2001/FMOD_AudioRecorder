...
void AudioManager::EndRecording()
{
	...
	// Took below code from class work
	std::ofstream file("recordedSound.wav", std::ios::binary);

	// Define WAV header fields
	size_t dataSize = m_RingBuffer.GetSize() * sizeof(float); // Size of the data to be written
	unsigned int fileSize = 36 + dataSize; // RIFF header size + data size
	float SAMPLE_RATE = 44100;  // Sampling rate for your recorded sound
	short channels = 1;         // Mono audio (use 2 for stereo)
	short bitsPerSample = 32;   // 32-bit float audio
	...
	short audioFormat = 3; // For floating-point audio (FMOD uses 32-bit floating point)
	...
	// Write the recorded audio data from the ring buffer
	file.write(reinterpret_cast<const char*>(m_RingBuffer.GetData(dataSize)), dataSize);

	file.close();

	std::cout << "Sound recording saved to project directory folder --> finalexam" << std::endl;
}
