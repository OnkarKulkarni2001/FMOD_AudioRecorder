...
void AudioManager::EndRecording()
{
	...
	std::ofstream outFile("recorded_audio.wav", std::ios::binary);
	if (!outFile.is_open())
	{
	...
	const uint32_t sampleRate = 44100; // Example: Adjust based on your setup
	const uint16_t channels = 1;      // Mono
	const uint16_t bitsPerSample = 16; // 16-bit samples
	const size_t bufferSize = 1024;   // Adjust as needed

	// Placeholder to calculate total data size
	size_t totalDataSize = 0;

	// Temporary buffer to read from the ring buffer
	int16_t tempBuffer[bufferSize];

	// Read data from RingBuffer and calculate total size
	while (/* Logic to check if RingBuffer has data */)
	{
		size_t availableData = /* Get available data size in RingBuffer */;
		size_t toRead = std::min(availableData, bufferSize);

		m_RingBuffer.Read(tempBuffer, toRead);
		outFile.write(reinterpret_cast<const char*>(tempBuffer), toRead * sizeof(int16_t));

		totalDataSize += toRead * sizeof(int16_t);
	}

	// Update WAV header with correct data size
	outFile.seekp(0, std::ios::beg);
	WriteWAVHeader(outFile, sampleRate, channels, bitsPerSample, totalDataSize);

	outFile.close();
	std::cout << "Recording saved to 'recorded_audio.wav'" << std::endl;
}
